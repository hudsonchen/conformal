...
class conformalMetalearner:
    ...
    def fit(self, X, W, Y, pscores):
        ...
        for i, (train_index, test_index) in enumerate(self.skf.split(W, W)):
            ...
            pscores_2 = self.pscores_models[i].predict_proba(X_2)[:, 1][:, None]
    ...
    def conformalize(self, alpha, X_calib, W_calib, Y_calib, oracle=None):
        ...
        for i in range(len(self.models_0)):
            ...
            pscores_calib = self.pscores_models[i].predict_proba(X_calib)[:, 1][:, None]

            y_hat_0_calibs.append(Y_hat_0_calib)
            y_hat_1_calibs.append(Y_hat_1_calib)
            pscores_calibs.append(pscores_calib)
        ...
        pscores_calibs  = np.mean(np.array(pscores_calibs), axis=0).reshape((-1,))

        Y_DR_calib      = self.get_pseudo_outcomes(W_calib, pscores_calibs, Y_calib, y_hat_0_calibs, y_hat_1_calibs, self.metalearner)
